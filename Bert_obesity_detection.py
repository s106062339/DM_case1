...
p_trainset = CustomDataset(tokenizer=tokenizer, path = "Train_p_" + str(half_len), mode = "Train")
h_trainset = CustomDataset(tokenizer=tokenizer, path = "Train_h_" + str(half_len), mode = "Train")
...
p_testset  = CustomDataset(tokenizer=tokenizer, path = "Test_p_" + str(half_len), mode = "Test")
h_testset  = CustomDataset(tokenizer=tokenizer, path = "Test_h_" + str(half_len), mode = "Test")
...
p_valset   = CustomDataset(tokenizer, path = "Validation_p_" + str(half_len), mode = "Val")
h_valset   = CustomDataset(tokenizer, path = "Validation_h_" + str(half_len), mode = "Val")
...
def p_train():
    ...
    for epoch in range(EPOCHS):
        ...
        if f1score > max_f1score:
            max_f1score = f1score
            torch.save(p_model, os.path.join(model_path, "P_model_" + str(half_len) + ".pt"))
...
def h_train():
    ...
    for epoch in range(EPOCHS):
        ...
        if f1score > max_f1score:
            max_f1score = f1score
            torch.save(h_model, os.path.join(model_path, "H_model_" + str(half_len) + ".pt"))
